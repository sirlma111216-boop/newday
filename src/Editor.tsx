...
export function Editor({ data, event, onSave, onClose }: { data: Data; event: Schedule; onSave: (event: Schedule, task: Task) => Promise<boolean>; onClose: () => void }) {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    if (task.links.some(link => !link.name.trim() || !safeLink(link.url))) {
      setError('자료 링크의 표시 이름과 주소를 확인해 주세요. https: 또는 onenote: 주소만 저장할 수 있습니다.');
      setAdvanced(true);
      return;
    }
    if (task.checklist.some(item => !item.text.trim())) { setError('준비할 일을 입력하거나 빈 항목을 삭제해 주세요.'); return; }
    const savedTask = { ...task, name: task.name.trim() || normalized.title.trim() };
    setSaving(true);
    try {
      if (await onSave({ ...normalized, title: normalized.title.trim(), taskId: savedTask.id }, savedTask)) onClose();
  ...
  return <Modal title={existing ? '일정 수정' : '새 일정'} onClose={close} wide>
    <form onSubmit={submit}>
      <fieldset disabled={saving} className="editor-fields"><div className="form-body">
        <label className="field">일정명<input autoFocus value={draft.title} maxLength={300} placeholder="어떤 일정이 있나요?" onChange={e => update({ title: e.currentTarget.value })}/></label>
    ...
    {discard && <div className="discard-box" role="alert"><p>저장하지 않은 변경 사항을 버릴까요?</p><button type="button" onClick={() => setDiscard(false)}>계속 작성</button><button type="button" onClick={onClose}>변경 버리기</button></div>}
  </Modal>;
}
