...
import { X, Plus, Trash2, Link2, CheckSquare, ChevronDown, ExternalLink } from 'lucide-react';
...
export function Editor({ data, event, onSave, onClose, mode = 'modal', onDelete }: { data: Data; event: Schedule; onSave: (event: Schedule, task: Task) => Promise<boolean>; onClose: () => void; mode?: 'modal' | 'panel'; onDelete?: () => void }) {
  ...
  const formBody = <fieldset disabled={saving} className="editor-fields"><div className="form-body">
    ...
    {advanced && <>
      ...
      <section className="form-section"><h3><Link2 size={17}/>관련 자료</h3>{task.links.map((link, index) => <div className="link-edit" key={link.id}><div><input aria-label={`링크 ${index + 1} 표시 이름`} placeholder="표시 이름" maxLength={300} value={link.name} onChange={e => changeTask({ links: task.links.map(item => item.id === link.id ? { ...item, name: e.currentTarget.value } : item) })}/><input aria-label={`링크 ${index + 1} 주소`} placeholder="https:// 또는 onenote:" maxLength={10000} value={link.url} onChange={e => changeTask({ links: task.links.map(item => item.id === link.id ? { ...item, url: e.currentTarget.value } : item) })}/></div>{safeLink(link.url) && <a className="icon-button" href={link.url} target={link.url.toLowerCase().startsWith('https:') ? '_blank' : undefined} rel="noopener noreferrer" aria-label={`링크 ${index + 1} 열기`}><ExternalLink size={16}/></a>}<button type="button" className="icon-button" aria-label={`링크 ${index + 1} 삭제`} onClick={() => changeTask({ links: task.links.filter(item => item.id !== link.id) })}><Trash2 size={16}/></button></div>)}<button type="button" className="text-button" onClick={() => changeTask({ links: [...task.links, { id: uid(), name: '', url: '' }] })}><Plus size={16}/>링크 추가</button><p className="help">원노트에서 ‘단락 링크 복사’ 후 원래 주소 전체를 붙여 넣으세요.<br/>원노트 설치 및 접근 권한에 따라 열리는 방식이 달라질 수 있습니다.</p></section>
