export const CATEGORIES = ['수업', '학교업무', '연수', '개인'] as const;
...
export function blankTask(): Task { return { id: uid(), name: '', category: '학교업무', status: '예정', memo: '', links: [], checklist: [] }; }
...
export function dueNotices(data: Data, now = Date.now()): Notice[] { const sent = new Set(data.delivered); return data.events.flatMap(event => { if (data.tasks.find(t => t.id === event.taskId)?.status === '완료') return []; return event.reminders.flatMap(offset => { const dueAt = eventTimestamp(event) - offset * 60000; const id = `${event.id}:${event.revision}:${event.reminderBase}:${offset}`; if (dueAt > now || dueAt < event.reminderSince || sent.has(id)) return []; return [{ id, eventId: event.id, title: `${event.title} · ${REMINDERS.find(r => r.value === offset)!.label}`, dueAt, read: false }]; }); }); }
